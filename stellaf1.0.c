...
CryptVerifySignature/*追加するstrings一覧
POST
UnregisterHotKey
...
void http(char *argv[],int *blacklist);
...
---ver1.0で追加するstrings一覧
139 POST
140 UnregisterHotKey


141 CreateToolhelp32Snapshot
142 FindWindowEx
143 Wow64DisableWow64FsRedirection
144 GetLogicalDriveStrings
145 ConvertStringSecurityDescriptorToSecurityDescriptor
146 LookupPrivilegeValue
147 NetLocalGroupAddMembers
148 freeaddrinfo
149 getaddrinfo
150 RemoveDirectory
...
ランサムウェアの可能性があります。
ファイルが暗号化されるかのうせいがあります。
170 CryptVerifySignature
171 CryptAcquireContext
172 CryptDestroyKey
173 CryptDecrypt
174 CryptHashData
175 CryptDestroyHash
176 CryptCreateHash
177 CryptEncrypt
178 CryptImportKey
179 CryptGetProvParam
180 CryptGetKeyParam
181 CryptReleaseContext
182 CryptDuplicateHash
...
/*---------------------------------------------------------------------------ver1.0で追加*/
    char *others139="POST\n";
    char *others140="UnregisterHotKey\n";
    char *others141="CreateToolhelp32Snapshot\n";
    char *others142="FindWindowEx\n";
    char *others143="Wow64DisableWow64FsRedirection\n";
    char *others144="GetLogicalDriveStrings\n";
    char *others145="ConvertStringSecurityDescriptorToSecurityDescriptor\n";
    char *others146="LookupPrivilegeValue\n";
    char *others147="NetLocalGroupAddMembers\n";
    char *others148="freeaddrinfo\n";
    char *others149="getaddrinfo\n";
    char *others150="RemoveDirectory\n";
    char *others151="http\n";

    char *others160="LockFileEx\n";
    char *others161="UnLockFileEx\n";

    char *others170="CryptVerifySignature\n";
    char *others171="CryptAcquireContext\n";
    char *others172="CryptDestroyKey\n";
    char *others173="CryptDecrypt\n";
    char *others174="CryptHashData\n";
    char *others175="CryptDestroyHash\n";
    char *others176="CryptCreateHash\n";
    char *others177="CryptEncrypt\n";
    char *others178="CryptImportKey\n";
    char *others179="CryptGetProvParam\n";
    char *others180="CryptGetKeyParam\n";
    char *others181="CryptReleaseContext\n";
    char *others182="CryptDuplicateHash\n";
    char *others183="CryptDuplicateHash\n";
    ...
    while(fgets(buf,BUFFER_SIZE,fp)!=NULL){
        ...
        if(strcmp(others139,others_conf)==0){
            blacklist[139]=1;
            printf(RED "%s",others139);
            printf(RESET);
        }
        if(strcmp(others140,others_conf)==0){
            blacklist[140]=1;
            printf(RED "%s",others140);
            printf(RESET);
        }
        if(strcmp(others141,others_conf)==0){
            blacklist[141]=1;
            printf(RED "%s",others141);
            printf(RESET);
        }
        if(strcmp(others142,others_conf)==0){
            blacklist[142]=1;
            printf(RED "%s",others142);
            printf(RESET);
        }
        if(strcmp(others143,others_conf)==0){
            blacklist[143]=1;
            printf(RED "%s",others143);
            printf(RESET);
        }
        if(strcmp(others144,others_conf)==0){
            blacklist[144]=1;
            printf(RED "%s",others144);
            printf(RESET);
        }
        if(strcmp(others145,others_conf)==0){
            blacklist[145]=1;
            printf(RED "%s",others145);
            printf(RESET);
        }
        if(strcmp(others146,others_conf)==0){
            blacklist[146]=1;
            printf(RED "%s",others146);
            printf(RESET);
        }
        if(strcmp(others147,others_conf)==0){
            blacklist[147]=1;
            printf(RED "%s",others147);
            printf(RESET);
        }
        if(strcmp(others148,others_conf)==0){
            blacklist[148]=1;
            printf(RED "%s",others148);
            printf(RESET);
        }
        if(strcmp(others149,others_conf)==0){
            blacklist[149]=1;
            printf(RED "%s",others149);
            printf(RESET);
        }
        if(strcmp(others150,others_conf)==0){
            blacklist[150]=1;
            printf(RED "%s",others150);
            printf(RESET);
        }
        if(strcmp(others151,others_conf)==0){
            blacklist[151]=1;
            printf(RED "%s",others151);
            printf(RESET);
        }
        if(strcmp(others160,others_conf)==0){
            blacklist[160]=1;
            printf(RED "%s",others160);
            printf(RESET);
        }
        if(strcmp(others161,others_conf)==0){
            blacklist[161]=1;
            printf(RED "%s",others161);
            printf(RESET);
        }
        if(strcmp(others170,others_conf)==0){
            blacklist[170]=1;
            printf(RED "%s",others170);
            printf(RESET);
        }
        if(strcmp(others171,others_conf)==0){
            blacklist[171]=1;
            printf(RED "%s",others171);
            printf(RESET);
        }
        if(strcmp(others172,others_conf)==0){
            blacklist[172]=1;
            printf(RED "%s",others172);
            printf(RESET);
        }
        if(strcmp(others173,others_conf)==0){
            blacklist[173]=1;
            printf(RED "%s",others173);
            printf(RESET);
        }
        if(strcmp(others174,others_conf)==0){
            blacklist[174]=1;
            printf(RED "%s",others174);
            printf(RESET);
        }
        if(strcmp(others175,others_conf)==0){
            blacklist[175]=1;
            printf(RED "%s",others175);
            printf(RESET);
        }
        if(strcmp(others176,others_conf)==0){
            blacklist[176]=1;
            printf(RED "%s",others176);
            printf(RESET);
        }
        if(strcmp(others177,others_conf)==0){
            blacklist[177]=1;
            printf(RED "%s",others177);
            printf(RESET);
        }
        if(strcmp(others178,others_conf)==0){
            blacklist[178]=1;
            printf(RED "%s",others178);
            printf(RESET);
        }
        if(strcmp(others179,others_conf)==0){
            blacklist[179]=1;
            printf(RED "%s",others179);
            printf(RESET);
        }
        if(strcmp(others180,others_conf)==0){
            blacklist[180]=1;
            printf(RED "%s",others180);
            printf(RESET);
        }
        if(strcmp(others181,others_conf)==0){
            blacklist[181]=1;
            printf(RED "%s",others181);
            printf(RESET);
        }
        if(strcmp(others182,others_conf)==0){
            blacklist[182]=1;
            printf(RED "%s",others182);
            printf(RESET);
        }
        if(strcmp(others183,others_conf)==0){
            blacklist[183]=1;
            printf(RED "%s",others183);
            printf(RESET);
        }
...
/*
    ...
    char *others161="UnLockFileEx\n";
*/
if(blacklist[160]==1 | blacklist[161]==1){
        printf(UNDER"\nファイルをロックされる可能性があります。"RESET);

        num=0;
        sum=2;
        num=blacklist[160]+blacklist[161];
        printf("%d/%d\n",num,sum);
       if(blacklist[160]==1){
           printf("%s",others160);
       }
...
/*
    ...
    char *others183="CryptDuplicateHash\n";
*/
if(blacklist[170]==1 | blacklist[171]==1 | blacklist[172]==1 | blacklist[173]==1 | blacklist[174]==1 | blacklist[175]==1 | blacklist[176]==1 | blacklist[177]==1 | blacklist[178]==1 | blacklist[180]==1 | blacklist[181]==1 | blacklist[182]==1 | blacklist[183]==1){
        printf(UNDER"\nファイルが暗号化される可能性があります。"RESET);

        num=0;
        sum=14;
        num=blacklist[170]+blacklist[171]+blacklist[172]+blacklist[173]+blacklist[174]+blacklist[175]+blacklist[176]+blacklist[177]+blacklist[178]+blacklist[179]+blacklist[180]+blacklist[181]+blacklist[182]+blacklist[183];
...
if(blacklist[160]==1 | blacklist[161]==1 | blacklist[170]==1 | blacklist[171]==1 | blacklist[172]==1 | blacklist[173]==1 | blacklist[174]==1 | blacklist[175]==1 | blacklist[176]==1 | blacklist[177]==1 | blacklist[178]==1 | blacklist[180]==1 | blacklist[181]==1 | blacklist[182]==1 | blacklist[183]==1){
        printf(UNDER"\nランサムウェアの可能性があります。"RESET);

        num=0;
        sum=16;
        num=blacklist[160]+blacklist[161]+blacklist[170]+blacklist[171]+blacklist[172]+blacklist[173]+blacklist[174]+blacklist[175]+blacklist[176]+blacklist[177]+blacklist[178]+blacklist[179]+blacklist[180]+blacklist[181]+blacklist[182]+blacklist[183];
